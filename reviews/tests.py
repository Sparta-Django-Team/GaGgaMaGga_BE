...
class CommentDetailViewTest(APITestCase):
    ...
    def test_comment_edit_success(self):
        response = self.client.put(
            path=reverse(
                "comment_detail_view", kwargs={"review_id": 1, "comment_id": 1}
            ),
            HTTP_AUTHORIZATION=f"Bearer {self.access_token_1}",
    ...
    def test_comment_same_author_report_fail(self):
        response = self.client.post(
            path=reverse(
                "comment_detail_view", kwargs={"review_id": 1, "comment_id": 2}
            ),
            HTTP_AUTHORIZATION=f"Bearer {self.access_token_1}",
            data=self.report_data,
        )
...
class RecommentAPIViewTestCase(APITestCase):
    ...
    def test_recommnet_list_success(self):
        response = self.client.get(
            path=reverse(
                "recomment_list_view", kwargs={"review_id": 1, "comment_id": 1}
            ),
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
        )
        self.assertEqual(response.status_code, 200)
...
class RecommentDetailViewTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.recomment = Recomment.objects.create(
            content="내용", 
            author=cls.user1, 
            comment=cls.comment,
        )
        cls.recomment2 = Recomment.objects.create(
            content="내용", 
            author=cls.user1, 
            comment=cls.comment,
        )
    ...
    def test_recomment_edit_success(self):
        response = self.client.put(
            path=reverse(
                "recomment_detail_view",
                kwargs={"review_id": 1, "comment_id": 1, "recomment_id": 1},
            ),
            HTTP_AUTHORIZATION=f"Bearer {self.access_token_1}",
            data={"content": "edit content"},
        )
    ...
    def test_recomment_report_already_reported_fail(self):
        response = self.client.post(
            path=reverse(
                "recomment_detail_view",
                kwargs={"review_id": 1, "comment_id": 1, "recomment_id": 2},
            ),
            HTTP_AUTHORIZATION=f"Bearer {self.access_token_2}",
            data=self.report_data,
        )
