...
class UserLoginLogoutAPIViewTestCase(APITestCase):
    @classmethod
    def setUpTestData(self):
        self.user_success_data = {"username": "test1234", "password": "Test1234!"}
        self.user_fail_data = {"username": "test1234", "password": "Test1234!!"}
    ...
    def setUp(self):
        ...
        self.refresh_token = self.client.post(reverse("token_obtain_pair_view"), self.user_success_data).data["refresh"]
        
    # (access_token)로그인 성공
    def test_access_token_login_success(self):
        response = self.client.post(reverse("token_obtain_pair_view"), self.user_success_data)
        self.assertEqual(response.status_code, 200)

    # (access_token)로그인 실패
    def test_access_token_login_fail(self):
        response = self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        self.assertEqual(response.status_code, 401)

    # (accss_token 여러번 시도)로그인 실패
    def test_access_token_login_attempt_fail(self):
        self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        self.client.post(reverse("token_obtain_pair_view"), self.user_fail_data)
        response = self.client.post(
            reverse("token_obtain_pair_view"), self.user_success_data
        )
        self.assertEqual(response.status_code, 400)
...
class UserConfirmPhoneNumberAPIViewTest(APITestCase):
    ...
    def test_confirm_phone_number_auth_number_invalid_fail(self):
        response = self.client.post(
            path=reverse("confirm_phone_number_view"),
            data={"phone_number": self.user.phone_number, "auth_number": 1234},
        )
        self.assertEqual(response.status_code, 400)
...
class ExpiredPasswordChageAPIView(APITestCase):
    @classmethod
    def setUpTestData(self):
        self.uesr_data = {"username": "test12341", "password": "Test1234!"}
        self.user = User.objects.create_user("test12341", "test1@test.com", "01012351234", "Test1234!")
    ...
    def setUp(self):
        self.access_token = self.client.post(reverse("token_obtain_pair_view"), self.uesr_data).data["access"]
